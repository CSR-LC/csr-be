...
func (s *CheckupTestSuite) TestCheckup_Success_PartialUpdate() {
	...
	s.orderFilterRepo.On("OrdersByStatus", mock.Anything, domain.OrderStatusInProgress, math.MaxInt64, 0, utils.AscOrder, order.FieldID).Return(list, nil)
...
func (s *CheckupTestSuite) TestCheckup_EmptyListOfOrders() {
	...
	s.orderFilterRepo.On("OrdersByStatus", mock.Anything, domain.OrderStatusInProgress, math.MaxInt64, 0, utils.AscOrder, order.FieldID).Return(nil, nil)
	s.setupApi.Checkup(ctx, client, observedLogger)
...
func (s *CheckupTestSuite) TestCheckup_OrderFilter_RepoErr() {
	...
	s.orderFilterRepo.On("OrdersByStatus", mock.Anything, domain.OrderStatusInProgress, math.MaxInt64, 0, utils.AscOrder, order.FieldID).Return(nil, err)
	s.setupApi.Checkup(ctx, client, observedLogger)
...
func (s *CheckupTestSuite) TestPeriodicalCheckup_EmptyListOfOrders() {
	t := s.T()
	...
	defer cancelFunc()
	s.setupApi.PeriodicalCheckup(ctx, time.Millisecond, client, observedLogger)
			...
			EquipmentStatus: []*ent.EquipmentStatus{
				{
					...
					Edges: ent.EquipmentStatusEdges{
						EquipmentStatusName: &ent.EquipmentStatusName{
							Name: domain.EquipmentStatusInUse,
						},
					},
				},
			},
