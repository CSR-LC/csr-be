...
func (s *OrderSuite) TestDeleteOrderRepository_Delete_OK() {
	t := s.T()
	ctx := s.ctx
	crtx, err := s.client.Tx(ctx)
	require.NoError(t, err)
	...
	err = s.orderRepository.Delete(ctx, createdOrder.ID)
	require.NoError(t, err)
...
func (s *OrderSuite) TestDeleteOrderRepository_Delete_NotFound() {
	t := s.T()
	ctx := s.ctx
	crtx, err := s.client.Tx(ctx)
	require.NoError(t, err)
	...
	err = s.orderRepository.Delete(ctx, 999)
